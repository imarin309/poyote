import { buildQualitySteps, MAX_OUTPUT_BYTES } from '../utils/imageQuality'
...
async function encodeWithMime(
  ...
  // 下限品質でも上限を超える場合は、その時点でもっとも小さい結果を返す（200KBは最善努力）
  return smallest
}

// 出力形式は「指定した形式」ではなく「実際に生成できた形式」で決まる。
// 拡張子は呼び出し元がblob.typeから決めること
export async function encodeCanvasWithinSize(
  canvas: HTMLCanvasElement,
